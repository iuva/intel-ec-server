...
def _log_service_error(func_name: str, error: Exception, args: tuple, kwargs: dict) -> None:
    ...
    # ✅ Log complete stack trace information (explicitly pass exception object and stack trace)
    logger.error(
        error_msg,
        extra=extra,
        exc_info=(type(error), error, error.__traceback__),  # Explicitly pass exception type, object and stack trace
    )
...
def _build_error_response_kwargs(error: BusinessError, api_locale: Optional[str] = None) -> Dict[str, Any]:
    ...
    if error.message_key:
        ...
        # Extract formatting variables (e.g., host_id) from details, pass to ErrorResponse
