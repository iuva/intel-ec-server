...
class SysConf(BaseDBModel):
    ...
    Inherits the following fields from BaseDBModel:
    ...
    conf_ver: Mapped[Optional[str]] = mapped_column(VARCHAR(32), nullable=True, comment="Configuration version number")
    ...
    # Status;{enable_flag: 0, enabled. disable_flag: 1, disabled.}
    state_flag: Mapped[int] = mapped_column(
        SmallInteger,
        default=0,
        nullable=False,
        index=True,
        comment="Status;{enable_flag: 0, enabled. disable_flag: 1, disabled.}",
    )

    # Created by
    created_by: Mapped[Optional[int]] = mapped_column(BigInteger, nullable=True, comment="Created by")

    # Updated by
    updated_by: Mapped[Optional[int]] = mapped_column(BigInteger, nullable=True, comment="Updated by")
