...
from sqlalchemy import BigInteger, DateTime, INT, SmallInteger, VARCHAR
...
class HostRec(BaseDBModel):
    ...
    host_no: Mapped[Optional[str]] = mapped_column(
        ...
        comment="主机主键;对应 mongo 数据库 host 主键"
    )

    # 唯一引导 id
    mg_id: Mapped[Optional[str]] = mapped_column(
        VARCHAR(128), nullable=True, comment="唯一引导id"
    )

    # 硬件记录表主键;host_hw_rec 表主键
    hw_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True,
        comment="硬件记录表主键;host_hw_rec 表主键"
    )

    # mongodb 主键;mongo db 硬件 id
    hardware_id: Mapped[Optional[str]] = mapped_column(
        VARCHAR(64), nullable=True,
        comment="mongodb 主键;mongo db 硬件 id"
    )

    # ip 地址
    host_ip: Mapped[Optional[str]] = mapped_column(
        VARCHAR(32), nullable=True, comment="ip 地址"
    )

    # ip 端口
    host_port: Mapped[Optional[int]] = mapped_column(
        INT, nullable=True, comment="ip 端口"
    )

    # 主机账号
    host_acct: Mapped[Optional[str]] = mapped_column(
        VARCHAR(32), nullable=True, comment="主机账号"
    )

    # 主机密码
    host_pwd: Mapped[Optional[str]] = mapped_column(
        VARCHAR(64), nullable=True, comment="主机密码"
    )

    # mac 地址
    mac_addr: Mapped[Optional[str]] = mapped_column(
        VARCHAR(255), nullable=True, comment="mac 地址"
    )
    ...
    appr_state: Mapped[Optional[int]] = mapped_column(
        ...
        comment=(
            "审批状态;{disable: 0, 停用. enable: 1, 启用. "
            "new: 1, 新增. change: 2, 存在改动.}"
        ),
    )
    ...
    host_state: Mapped[Optional[int]] = mapped_column(
        SmallInteger, nullable=True,
        comment=(
            "主机状态;0-空闲 1-已锁定 2-已占用 3-case执行中 "
    ...
    subm_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="申报时间"
    )

    # agent 版本号
    agent_ver: Mapped[Optional[str]] = mapped_column(
        VARCHAR(10), nullable=True, comment="agent 版本号"
    )
