...
async def delete_host(
    ...
    # Call service layer to delete (***REMOVED*** request object to get user_id from request headers)
...
async def force_offline_host(
    ...
    # Call service layer to force offline (***REMOVED*** locale parameter for multilingual error messages)
...
@router.put(
    "/***REMOVED***word",
    response_model=SuccessResponse,
    summary="Update host ***REMOVED***word",
    description="Update host ***REMOVED***word (stored after AES encryption)",
    responses={
        200: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "code": 200,
                        "message": "Host ***REMOVED***word updated successfully",
                        "data": {
                            "id": "123",
                        },
                    }
                ...
                "application/json": {
                    "examples": {
                        ...
                        "***REMOVED***word_update_failed": {
                            ...
                            "value": {
                                "code": 53006,
                                "message": "Host ***REMOVED***word update failed, record may have been deleted (ID: 123)",
...
async def update_host_***REMOVED***word(
    request: AdminHostUpdatePasswordRequest = Body(..., description="Host ***REMOVED***word update request data"),
    admin_host_service: AdminHostService = Depends(get_admin_host_service),
    current_user: dict = Depends(get_current_user),
    locale: str = Depends(get_locale),
) -> SuccessResponse:
    ...
    Business logic:
    1. Check if host exists and is not deleted
    2. Encrypt received plaintext ***REMOVED***word with AES
    3. Update host_rec table host_pwd field to encrypted ***REMOVED***word

    Args:
        request: Request object containing host ID and plaintext ***REMOVED***word
        admin_host_service: Admin backend host service instance
        current_user: Current user information
        locale: Language preference
    ...
    Raises:
        BusinessError: When host does not exist, ***REMOVED***word encryption fails, or update fails
    """
    logger.info(
        "Received admin backend host ***REMOVED***word update request",
        extra={
            "host_id": request.host_id,
            "user_id": current_user.get("id"),
        },
