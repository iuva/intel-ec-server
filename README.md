...
#### ✅ Gateway WebSocket 错误处理优化 - 精准识别认证失败

**问题**: Gateway 转发 WebSocket 时，后端返回 403（认证失败），但 Gateway 返回通用的 500 错误和模糊的错误消息。

**修复方案**:
1. **新增错误码**: `GATEWAY_AUTH_FAILED = 10011`（403）, `GATEWAY_UNAUTHORIZED = 10012`（401）
2. **改进异常处理**: 检查错误消息中的 HTTP 状态码，返回准确的错误信息
3. **日志优化**: 认证失败使用 WARNING 级别，其他错误使用 ERROR 级别

**错误响应对比**:
- **Before**: `{"code": 500, "message": "WebSocket 转发异常"}`
- **After**: `{"code": 403, "message": "WebSocket 认证失败，Token 无效或已过期"}`

**优势**:
- ✅ 准确的 HTTP 状态码（403/401 而不是 500）
- ✅ 清晰的错误消息提示
- ✅ 便于客户端判断错误类型并采取相应措施

详情: [WEBSOCKET_ERROR_HANDLING_FIX.md](WEBSOCKET_ERROR_HANDLING_FIX.md)

---

#### ✅ WebSocket 认证优化 - 从 Token 获取 host_id

**问题**: WebSocket 端点通过 URL 参数传递 `host_id`，存在安全风险和规范性问题。

**优化方案**:
1. **Token 验证**: `admin_login` 和 `device_login` 的 token 中 `sub` 字段已正确存储 ID
2. **IntrospectResponse 扩展**: 新增 `user_type`, `mg_id`, `host_ip`, `sub` 字段
3. **introspect_token 优化**: 返回完整的 token payload 信息
4. **WebSocket 端点重构**: 路径从 `/ws/host/{host_id}` 改为 `/ws/host`，host_id 从 token 中提取

**连接示例**:
- **直接连接**: `ws://localhost:8003/api/v1/ws/host?token=xxx`
- **通过Gateway**: `ws://localhost:8000/ws/host-service/host?token=xxx`

**优势**:
- ✅ 更安全: host_id 不暴露在 URL 中
- ✅ 更规范: 完全基于 JWT 认证标准
- ✅ 更简洁: URL 更简单明了
- ✅ 完整信息: Token 包含所有用户上下文

详情: [WEBSOCKET_AUTH_OPTIMIZATION.md](WEBSOCKET_AUTH_OPTIMIZATION.md)

---

#### ✅ WebSocket 心跳监控优化 - 解决数据库查询失败

**问题**: 心跳监控调用 `get_host_by_id(agent_id)` 时，因为 agent_id 是字符串（如 `agent-123`）无法转换为整数导致查询失败。

**修复方案**:
1. **增加内存心跳时间戳跟踪**: `heartbeat_timestamps: Dict[str, datetime]`
2. **优化心跳处理逻辑**: 优先更新内存，数据库更新失败不影响监控
3. **重构心跳监控任务**: 直接检查内存时间戳，不依赖数据库查询
4. **完善资源清理**: disconnect 时清理 heartbeat_timestamps

**优势**:
- ✅ 支持任意格式的 agent_id（字符串、整数、UUID等）
- ✅ 心跳监控不再依赖数据库查询
- ✅ 数据库异常不影响WebSocket连接
- ✅ 更好的性能和可靠性

详情: [WEBSOCKET_HEARTBEAT_FIX.md](WEBSOCKET_HEARTBEAT_FIX.md)

---

#### ✅ WebSocket 消息转发完全修复 - 处理两种WebSocket对象类型

**WebSocket消息转发修复历程**:

1. **第一次修复** (b02ef06): 移除async for错误
   - 问题: `'async for' requires an object with __aiter__ method, got WebSocket`
   - 修复: 使用 `while True` + `receive_text()` / `receive_bytes()` 替代async for

2. **第二次修复** (2e40d44): 区分两种WebSocket类型 ⭐ **完全解决**
   - 问题: `'WebSocketClientProtocol' object has no attribute 'receive_text'`
   - 根因: Gateway中存在两种WebSocket对象:
     - 客户端 → Gateway: FastAPI `WebSocket` (有 `receive_text()` / `send_text()`)
     - Gateway → 后端: `websockets.WebSocketClientProtocol` (有 `recv()` / `send()`)
   - 修复: 使用类型检测选择正确的API
     ```python
     is_fastapi_destination = hasattr(destination, 'send_text')  # 准确检测
     if is_fastapi_destination:
         await destination.send_text(message)  # FastAPI API
     else:
         await destination.send(message)  # websockets API
     ```

**WebSocket完整流程验证**:
- ✅ Token验证成功 (verify_token_string)
- ✅ WebSocket连接建立 (连接到Gateway)
- ✅ 后端WebSocket连接建立 (连接到Host-Service)
- ✅ **消息双向正确转发** (已修复)

**关键日志**:
```log
INFO | Token 字符串验证成功
INFO | WebSocket 连接认证成功
INFO | WebSocket 连接已建立: host/agent/agent-123
INFO | 转发 WebSocket 连接
INFO | 后端 WebSocket 连接已建立
# ✅ 消息正确转发（不再报错）
INFO | WebSocket 连接已关闭
```

详见 [WEBSOCKET_COMPLETE_FIX.md](./WEBSOCKET_COMPLETE_FIX.md)

#### ✅ WebSocket 403 问题完全修复 - 修复Gateway URL转发和Auth-Service路由

**第一层修复**: Auth-Service路由前缀错误
- 文件: `services/auth-service/app/api/v1/__init__.py`
- 修复: `prefix=""` → `prefix="/auth"`

**第二层修复**: Gateway URL转发缺少服务标识符 ⭐ **关键修复**
- 文件: `services/gateway-service/app/services/proxy_service.py`
- 问题: 转发 `/api/v1/auth/admin/login` 时，生成的后端URL为 `/api/v1/admin/login` (缺少 `/auth`)
- 修复: `_build_service_url()` 添加service_name参数，生成正确的URL: `/api/v1/auth/admin/login`

**修复结果**: 
- ✅ Gateway可正常转发HTTP请求到各个微服务
- ✅ `http://localhost:8000/api/v1/auth/admin/login` 现在返回 **401** (认证失败) 而不是404
- ✅ WebSocket token验证流程已恢复正常
- ✅ introspect端点可正常调用

**验证测试**:
```bash
# HTTP请求转发测试 - 返回 401 (认证失败，而不是404)
curl -X POST http://localhost:8000/api/v1/auth/admin/login \
  -H "Content-Type: application/json" \
  -d '{"username":"test","***REMOVED***word":"test"}'
# Response: 401 "AUTH_INVALID_CREDENTIALS"

# 直接访问auth-service验证 - 同样返回401
...
详见 [FINAL_SOLUTION.md](./FINAL_SOLUTION.md) 和 [GATEWAY_AUTH_WHITELIST_ANALYSIS.md](./GATEWAY_AUTH_WHITELIST_ANALYSIS.md)

### 🔧 之前修复 (2025-10-25)

#### ✅ WebSocket 测试框架完整实现 - 29+ 测试用例全覆盖

**实现内容**:
完整的 WebSocket 接口测试框架，包含连接、消息、并发、性能四大测试模块。
...
2. **消息处理** (8个用例)
   - JSON验证、大消息(1MB)、特殊字符、嵌套结构等

3. **并发测试** (8个用例)
   - 多Agent连接(10个)、负载测试(50连接)、高频消息(500条)等

4. **性能测试** (6个用例)
   - 吞吐量(>100msg/s)、延迟(<50ms)、内存(<200MB)等
...
**文档**:
- `services/host-service/tests/README.md` - 完整框架说明
- `services/host-service/tests/QUICK_START.md` - 快速参考指南

**文件位置**: `services/host-service/tests/`
...
网关服务现已支持 **WebSocket 连接转发**，允许客户端通过网关访问后端微服务的 WebSocket 端点。

#### 核心特性
- ✅ **WebSocket 路由处理**: `/ws/{service_name}/{path}`
- ✅ **双向消息转发**: 客户端 ←→ 网关 ←→ 后端服务
- ✅ **连接管理**: 自动处理连接生命周期
- ✅ **错误处理**: 完整的异常处理和日志记录
...
# 通过网关连接到 host-service WebSocket
...
```bash
# 1. 启动网关和 host-service
docker-compose up -d gateway-service host-service

# 2. 运行 WebSocket 测试框架
cd services/host-service/tests
pytest test_websocket_connection.py -v

# 3. 或使用网关连接运行
export WEBSOCKET_URL="ws://localhost:8000/ws/host"
pytest test_websocket_connection.py -v
```

#### 文档
- 📖 [网关 WebSocket 使用指南](services/host-service/tests/GATEWAY_WEBSOCKET.md)
- 📖 [WebSocket 测试框架](services/host-service/tests/README.md)
- 📖 [快速参考](services/host-service/tests/QUICK_START.md)

#### 架构

```
客户端 --ws--> 网关 (8000) --ws--> Host Service (8003)
                   ↓
              双向消息转发
                   ↓
            /ws/{service}/{path}
...
通过网关访问微服务的 WebSocket 接口时，需要使用以下地址格式：

```
ws://localhost:8000/api/v1/ws/{service-name}/{api-path}
```

**参数说明**:
- `service-name`: 后端服务完整名称 (例如 `host-service`, `auth-service`, `admin-service`)
- `api-path`: API 路径 (例如 `agent/agent-123`, `device/device-456`)

**完整示例**:
```
✅ ws://localhost:8000/api/v1/ws/host-service/agent/agent-123
✅ ws://localhost:8000/api/v1/ws/host-service/device/device-456
✅ ws://localhost:8000/api/v1/ws/auth-service/session/session-001
✅ ws://localhost:8000/api/v1/ws/admin-service/config/config-001
```

### 🔐 WebSocket 认证

**WebSocket 连接需要认证令牌**。支持以下认证方式：

#### 方式 1: 查询参数（推荐）
```
ws://localhost:8000/api/v1/ws/host-service/agent/agent-123?token=<your-jwt-token>
```

#### 方式 2: Authorization 请求头
```javascript
const ws = new WebSocket('ws://localhost:8000/api/v1/ws/host-service/agent/agent-123');
ws.onopen = () => {
    ws.send(JSON.stringify({type: "auth", token: "<your-jwt-token>"}));
};
```

#### 方式 3: 自定义 X-Token 请求头
```python
import asyncio
import websockets

async def connect():
    uri = "ws://localhost:8000/api/v1/ws/host-service/agent/agent-123"
    headers = {"X-Token": "<your-jwt-token>"}
...
async def test_websocket():
    # ✅ 正确：提供 token
    uri = "ws://localhost:8000/api/v1/ws/host-service/agent/agent-123?token=<valid-token>"
...
**常见错误** ❌:
```
❌ HTTP 403: 缺少认证令牌 - 需要在 URL 或请求头中提供 token
❌ HTTP 403: 认证令牌无效或已过期 - token 不合法或已过期
❌ ws://localhost:8000/agent/agent-123 - 缺少 /api/v1/ws 前缀
❌ ws://localhost:8000/api/v1/host-service/agent/agent-123 - 缺少 /ws 前缀
```
...
async def connect_to_host_service():
    # 获取有效的 JWT token
    token = "your-valid-jwt-token"
    
    uri = f"ws://localhost:8000/api/v1/ws/host-service/agent/agent-123?token={token}"
    print(f"🚀 尝试连接: {uri}")
    ...
    try:
        async with websockets.connect(uri) as websocket:
            ...
            # 发送消息
            await websocket.send(json.dumps({"type": "ping", "data": "hello"}))
...
asyncio.run(connect_to_host_service())
```
...
export function WebSocketComponent() {
    ...
    useEffect(() => {
        ...
        const uri = `ws://localhost:8000/api/v1/ws/host-service/agent/agent-123?token=${token}`;
        
        console.log('🚀 连接 WebSocket:', uri);
